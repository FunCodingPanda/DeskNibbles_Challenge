...
const solve = (products, snackers) => {
  ...
  const fave_snacks = new Set();
  for (let i = 0; i < snackers.length; i++) {
    const snacker = snackers[i];
    fave_snacks.add(snacker.fave_snack);
  }
  ...
  const intersection = new Set([...fave_snacks].filter(x => product_names.has(x)));
...
fs.readFile('products.json', 'utf8', (err, data) => {
  ...
  fs.readFile('MOCK_SNACKER_DATA.json', 'utf8', (err, data) => {
